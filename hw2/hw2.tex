...
Apply the same logic we did for the "top" of the surface to the bottom, where we divide it into two segments and attach the mobius strip that way.
...
We can continously deform the surface where $a$ essentially becomes $0$ and it is still topologically equivalent: 
...
First, we will begin by proving a supplemental lemma. Begin with a mobius strip and Klein bottle with a hole.
...
Glue them together along the boundary of the Klein bottle, $f$.
...
Then, we can cut the shape as follows in order to reglue it along $b$.
...
Let the above surface be the result of our lemma. 
...
This is clearly homeomorphic to the shape we proved in our lemma, meaning the shape we are currently at is topological equivalent to the connected sum of a Möbius band and a Klein bottle. 

Next, we know that three projective planes are topologically equivalent to a Möbius band and a Klein bottle. That is also topologically equivalent to a Klein bottle and a projective plane, since $KB \cong P^2 \# P^2$. Thus, through proving this homeomorphism with the lemma, we have overall proved that hte surface is the connected sum of the Klein bottle and the projective plane, $\mathbb{P^2} \# KB$.
